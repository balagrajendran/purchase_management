...
const COL = firebase_1.db.collection("invoices");
...
router.get("/stats", async (req, res) => {
    ...
    if (dateFrom)
        items = items.filter((i) => i.createdAt >= dateFrom);
    if (dateTo)
        items = items.filter((i) => i.createdAt <= dateTo);
...
router.patch("/:id", async (req, res) => {
    ...
    if (Array.isArray(patch.items)) {
        subtotal = patch.items.reduce((s, it) => s + Number(it.total || 0), 0);
